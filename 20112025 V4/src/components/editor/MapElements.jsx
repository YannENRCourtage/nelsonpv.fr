...
// import { toPng } from 'html-to-image'; // DÉSACTIVÉ pour Horizons
...
// Standard Bearing (0=North, CW): N=0, E=90, S=180, W=270
// User wants: S=0 (Standard 180), W=90 (Standard 270), N=180 (Standard 0), E=-90 (Standard 90)
// Formula: (Standard_Bearing - 180) normalized to [-180, 180]
function calculateCustomAzimuth(a, b) {
  ...
  // Convert to 0-360 CW from North
  let standard = (brng + 360) % 360;

  // Apply shift: South (180) should be 0.
  // West (270) should be 90.
  // North (0) should be 180.
  // East (90) should be -90 (or 270).

  // Let's try: (Standard - 180)
  // N(0) -> -180. User wants 180. (-180 == 180 mod 360)
  // E(90) -> -90. User wants -90. OK.
  // S(180) -> 0. User wants 0. OK.
  // W(270) -> 90. User wants 90. OK.

  let custom = standard - 180;
  if (custom > 180) custom -= 360;
  if (custom <= -180) custom += 360;

  // Special case for exact North to return 180 instead of -180 if preferred, but -180/180 is same.
...
function ContextMenu({ position, onAddText, onClose, onShowInfo, onSetTarget }) {
  ...
  const copyCoords = () => {
    ...
    navigator.clipboard.writeText(coords).then(() => toast({ title: "Coordonnées copiées !", description: coords }));
    onClose();
  };
...
function EditLayer({ mode, setMode, features, setFeatures, temp, setTemp, selectedId, setSelectedId, askTextAt, setAskTextAt, symbolToPlace, setSymbolToPlace, setPointInfo, setAltimetryProfile, rectangleStart, setRectangleStart, photoToPlace, setPhotoToPlace, targetPos, setTargetPos }) {
  ...
  const handleAltimetry = async (line) => {
    ...
    try {
      ...
      toast({ title: "Erreur de profil altimétrique", description: "Impossible de récupérer les données d'altitude.", variant: "destructive" });
    }
  };
...
const LAYERS = {
  ...
  esriSat: { name: "Satellite Esri", url: "https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}", attrib: 'Tiles &copy; Esri', zIndex: 0 },
  osm: { name: "Plan OSM", url: "https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", attrib: '&copy; OpenStreetMap contributors', zIndex: 0 },
...
function MapTargetInfo({ targetPos, setTargetPos }) {
    ...
    const updateInfo = async () => {
      ...
      try {
        const [altRes, addrRes, parcRes] = await Promise.allSettled([
          fetch(`https://wxs.ign.fr/essentiels/alti/rest/elevation.json?lon=${targetPos.lng}&lat=${targetPos.lat}&zonly=true`).then(r => r.json()),
  ...
  const copyCoords = () => {
    ...
    navigator.clipboard.writeText(text).then(() => toast({ title: "Coordonnées copiées", description: text }));
  };
...
function MapEvents({ project, onAddressFound, onAddressSearched, setPhotoToPlace, onBuildingSelect, setFeatures }) {
  ...
  useEffect(() => {
    const handlePlaceBuilding = (e) => {
      ...
      toast({ title: `Bâtiment ${building.code} ajouté`, description: "Le bâtiment a été placé au centre de la carte." });
    };
  ...
  useEffect(() => {
    const handleCaptureRequest = async (e) => {
      ...
      if (map) {
        const drawingTools = map.getContainer().querySelector('.leaflet-control-container .leaflet-top.leaflet-left');
        if (drawingTools) drawingTools.style.display = 'none';
        const canvas = await html2canvas(map.getContainer(), {
          useCORS: true, logging: false,
          onclone: (doc) => {
            const controlsToHide = doc.querySelectorAll('.leaflet-pane.leaflet-control-container .leaflet-top.leaflet-left, .leaflet-control-container .leaflet-top.leaflet-right, .leaflet-control-container .leaflet-bottom.leaflet-right, .leaflet-control-container .leaflet-bottom.leaflet-left');
            ...
            if (leafletPane) {
              ...
              const parts = /translate3d\((.+)px, (.+)px, (.+)\)/.exec(transform);
              if (parts) {
                const x = parseFloat(parts[1]); const y = parseFloat(parts[2]);
                doc.querySelectorAll('.leaflet-pane > svg, .leaflet-pane .leaflet-marker-pane, .leaflet-pane .leaflet-tooltip-pane').forEach(pane => { pane.style.transform = `translate(${x}px, ${y}px)`; });
              }
            }
          }
        });
        if (drawingTools) drawingTools.style.display = '';
    ...
    return () => { window.removeEventListener("map:capture-request", handleCaptureRequest); window.removeEventListener("map:goto-project-address", goToProjectAddress); window.removeEventListener("map:place-photo", handlePlacePhoto); };
...
function AltimetryProfile({ profile, setProfile, setFeatures, features }) {
  ...
  const handleCloseProfile = () => {
    setProfile(null);
    toast({ title: "Profil altimétrique fermé", description: "Les informations du profil altimétrique ne sont plus affichées.", className: "text-black" });
  };

  /* TEMPORAIREMENT DÉSACTIVÉ - html-to-image non disponible dans Horizons
    ...
    if (chartRef.current) {
      toPng(chartRef.current, { cacheBust: true })
        .then((dataUrl) => {
          ...
          link.href = dataUrl;
          link.click();
          toast({ title: "Export réussi !", description: "Le profil a été exporté en PNG." });
        })
        .catch((err) => {
  ...
  */
  ...
  const handleExportCSV = () => {
    ...
    toast({ title: "Export CSV réussi !", description: "Les données ont été exportées." });
  };
  ...
  const handleZoomToProfile = () => {
    if (profile?.line && map) {
      ...
      toast({ title: "Zoom ajusté", description: "La carte est centrée sur le profil." });
    }
  };
