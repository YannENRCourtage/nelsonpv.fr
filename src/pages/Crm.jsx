...
export default function Crm() {
  ...
  const kpis = [
    ...
    },
    {
      ...
      value: `${Math.round(opportunities.reduce((sum, opp) => sum + opp.amount, 0) / 1000)}K€`,
      trend: '+23%',
      trendPositive: true,
      color: 'bg-purple-500',
  ...
  const recentActivities = [
    ...
    },
    {
      id: 2,
      name: 'Système',
      action: 'opportunité mise à jour',
      target: opportunities[0]?.name || 'Opportunité',
      time: 'Il y a 3h',
      avatar: 'S',
      color: 'bg-green-500'
    },
  ...
  const handleSaveOpportunity = () => {
    if (editingOpportunity.id && opportunities.find(o => o.id === editingOpportunity.id)) {
      setOpportunities(opportunities.map(o => o.id === editingOpportunity.id ? editingOpportunity : o));
    } else {
      setOpportunities([...opportunities, editingOpportunity]);
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-slate-900">Pipeline des ventes</h2>
            <button
          ...
          <div className="space-y-5">
            {opportunities.slice(0, 4).map((opp) => (
              <div key={opp.id} className="group hover:bg-slate-50 rounded-xl p-4 transition-all duration-200">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex-1">
                    <h3 className="font-semibold text-slate-900 mb-1">{opp.name}</h3>
                    <p className="text-sm text-slate-600">Contact: {opp.contact}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-slate-900 text-lg">{opp.amount.toLocaleString()} €</p>
                    <p className="text-sm text-slate-600">{opp.status}</p>
                  </div>
                </div>
                ...
                <div className="relative">
                  <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div
                      className={`h-full ${opp.color} rounded-full transition-all duration-500`}
                      style={{ width: `${opp.probability}%` }}
                    />
                  </div>
                  <span className="absolute -top-1 right-0 text-xs font-semibold text-slate-700">
                    {opp.probability}%
                  </span>
                </div>
              </div>
  ...
  // Rendu de la liste des Opportunités
  const renderOpportunities = () => (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-slate-900">Opportunités</h2>
        <Button
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-700 uppercase">Opportunité</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-700 uppercase">Contact</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-700 uppercase">Montant</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-700 uppercase">Probabilité</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-700 uppercase">Statut</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-700 uppercase">Date clôture</th>
              </tr>
            </thead>
              ...
              {opportunities.map((opp) => (
                <tr key={opp.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="font-semibold text-slate-900">{opp.name}</div>
                    <div className="text-sm text-slate-600">{opp.company}</div>
                  </td>
                  <td className="px-6 py-4 text-slate-700">{opp.contact}</td>
                  <td className="px-6 py-4 font-semibold text-slate-900">{opp.amount.toLocaleString()} €</td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden max-w-[100px]">
                        <div className={`h-full ${opp.color}`} style={{ width: `${opp.probability}%` }} />
                      </div>
                      <span className="text-sm font-semibold text-slate-700">{opp.probability}%</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${opp.color} bg-opacity-10 ${opp.color.replace('bg-', 'text-')}`}>
                      {opp.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-slate-700">{new Date(opp.closeDate).toLocaleDateString('fr-FR')}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="grid grid-cols-7 gap-2">
            ...
            {days.map((day, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className={`min-h-[80px] p-2 rounded-lg border ${day ? 'bg-white border-slate-200 hover:bg-slate-50' : 'bg-slate-50 border-transparent'
  ...
  // Modal de contact
  const ContactModal = () => {
    if (!showContactModal || !editingContact) return null;
  ...
  const OpportunityModal = () => {
    if (!showOpportunityModal || !editingOpportunity) return null;

    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          <div className="p-6 border-b border-slate-200 flex items-center justify-between sticky top-0 bg-white">
            <h3 className="text-xl font-bold text-slate-900">Nouvelle opportunité</h3>
            <button
              onClick={() => { setShowOpportunityModal(false); setEditingOpportunity(null); }}
              className="p-2 hover:bg-slate-100 rounded-lg transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          ...
          <div className="p-6 space-y-4">
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-2">Nom de l'opportunité</label>
              <input
                type="text"
                ...
                placeholder="Installation PV 100kWc"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setEditingOpportunity({ ...editingOpportunity, amount: parseInt(e.target.value) || 0 })}
                  placeholder="50000"
                />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Probabilité (%)</label>
                <input
                  type="number"
                  min="0"
                  max="100"
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Date de clôture</label>
                <input
                  type="date"
                  className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                  value={editingOpportunity.closeDate}
                  onChange={(e) => setEditingOpportunity({ ...editingOpportunity, closeDate: e.target.value })}
                />
              </div>
            </div>
          </div>
  ...
  // Modal de tâche
  const TaskModal = () => {
    if (!showTaskModal || !editingTask) return null;
        ...
        <div className="p-6">
          ...
          <div className="mb-8">
            ...
            <p className="text-slate-600">
              ...
              {activeTab === 'opportunities' && 'Suivez vos opportunités commerciales'}
