  ...
  const handleExportPDF = async (simToExport = null) => {
    ...
    if (isSechoirSolution) {
      ...
      targetSim = {
        ...
        subventionsPAE: 0,
        subventionRegionaleNom: simResults?.subventionsEligibles?.subventionRegionale?.nom || 'PCAE / PME - Nouvelle-Aquitaine',
        subventionDescription: simResults?.subventionsEligibles?.description || 'Plan de Modernisation des Exploitations.',
        subventionTauxTexte: simResults?.subventionsEligibles?.tauxTexte || '30% (+10% JA)',
        subventionRegionaleMontant: simResults?.subventionsEligibles?.montantEstime || 0,
        subventionPlafond: simResults?.subventionsEligibles?.subventionRegionale?.montantMax || 100000,
        roiBonifie: simResults?.roiBonifie,
        regionName: simResults?.subventionsEligibles?.region || 'Nouvelle-Aquitaine',
