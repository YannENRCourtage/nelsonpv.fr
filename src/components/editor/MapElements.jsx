...
function InstallationsProdLayerManager({ layersRef, activeLayers }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // 4. Rendu des marqueurs aux coordonnées réelles avec calcul de puissance précis
      osmElements.forEach(el => {
        const pLat = el.lat || el.center?.lat;
        const pLon = el.lon || el.center?.lon;
        if (!pLat || !pLon || isNaN(pLat) || isNaN(pLon)) return;

        // Calcul de la surface si polygone disponible
        let areaM2 = 0;
        ...
        // DÉTERMINATION PRÉCISE DU NOM / TYPE D'INSTALLATION
        let nomInst = '';
        if (odreItem?.nominstallation && odreItem.nominstallation !== 'Confidentiel') {
        ...
        const popupContent = `
          <div style="font-family: sans-serif; min-width: 250px; padding: 4px;">
            <div style="display: flex; align-items: center; justify-content: space-between; border-bottom: 1.5px solid #e2e8f0; padding-bottom: 5px; margin-bottom: 6px;">
              <span style="font-size: 13px; font-weight: 800; color: #0D3660; text-transform: uppercase;">⚡ ${isRoof ? 'Toiture Solaire' : 'Installation Prod.'}</span>
              <span style="font-size: 10px; font-weight: bold; background: #fef08a; color: #854d0e; padding: 2px 6px; border-radius: 4px;">${filiere}</span>
            </div>
            <div style="font-size: 13px; font-weight: bold; color: #0f172a; margin-bottom: 4px;">${nomInst}</div>
            ...
            ${areaM2 > 0 ? `<div style="font-size: 11px; color: #64748b; margin-bottom: 2px;">Surface estimée : <strong style="color: #334155;">~${areaM2.toFixed(0)} m²</strong></div>` : ''}
...
// ====================================================================
// LÉGENDE FEUX DE FORÊT (OLD / IGN DFCI)
// ====================================================================
function FeuxForetOLDLegend({ layersRef }) {
  const map = useMap();
  const [showLegend, setShowLegend] = useState(false);

  useEffect(() => {
    const checkLayer = () => {
      const layer = layersRef.current['feuxForetOLD'];
      setShowLegend(layer && map.hasLayer(layer));
    };
    checkLayer();
    const interval = setInterval(checkLayer, 500);
  ...
  return (
    <div
      className="absolute bottom-[268px] right-[10px] z-[995] bg-white/95 backdrop-blur-sm p-3 rounded-lg shadow-xl border border-gray-300 max-w-[250px]"
      style={{ userSelect: 'none' }}
    >
      <div className="flex justify-between items-center mb-2">
        <h4 className="font-bold text-xs text-gray-900 flex items-center gap-1.5">
          <span>🔥</span> Légende Feux de forêt (OLD)
        </h4>
        <button onClick={() => setShowLegend(false)} className="p-1 hover:bg-gray-200 rounded"><XIcon className="h-3 w-3" /></button>
      </div>
      <div className="space-y-1.5 text-[10px]">
        <div className="flex items-start gap-2">
          <div className="w-4 h-4 rounded bg-[#9333EA] opacity-75 border border-purple-800 shrink-0 mt-0.5"></div>
          <div>
            <span className="text-gray-900 font-semibold">Zone d'Obligation Légale (OLD)</span>
            <p className="text-gray-600 text-[9px] leading-tight">Périmètre à moins de 200m des massifs forestiers soumis à l'obligation de débroussaillement.</p>
          </div>
        </div>
      </div>
      <div className="mt-2 pt-1.5 border-t border-gray-200 text-[9px] text-gray-500">
...
// ====================================================================
// LÉGENDE PARKING >500M²
// ====================================================================
function ParkingLegend({ layersRef }) {
  const map = useMap();
  const [showLegend, setShowLegend] = useState(false);
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-2">
        <h4 className="font-bold text-xs text-gray-900">Légende Parkings</h4>
      ...
      <div className="space-y-1">
        <img 
          src="https://data.geopf.fr/wms-v/ows?service=WMS&version=1.3.0&request=GetLegendGraphic&format=image/png&layer=PARKING.SUP.500" 
...
function BanPlusLegend({ layersRef }) {
  const map = useMap();
  const [showLegend, setShowLegend] = useState(false);

  useEffect(() => {
    const checkLayer = () => {
      const layer = layersRef.current['banPlus'];
      setShowLegend(layer && map.hasLayer(layer));
    };
    checkLayer();
    const interval = setInterval(checkLayer, 500);
  ...
  return (
    <div
      className="absolute bottom-[520px] right-[10px] z-[995] bg-white/95 backdrop-blur-sm p-3 rounded-lg shadow-xl border border-gray-300 max-w-[210px]"
      style={{ userSelect: 'none' }}
    >
      <div className="flex justify-between items-center mb-2">
        <h4 className="font-bold text-xs text-gray-900">Légende BAN PLUS</h4>
        <button onClick={() => setShowLegend(false)} className="p-1 hover:bg-gray-200 rounded">
          <XIcon className="h-3 w-3" />
        </button>
      </div>
...
function MapTargetInfo({ targetPos, setTargetPos, hoverInfo, showInfoPanel, setShowInfoPanel, project, setProject }) {
  ...
  return (
    <div 
      ref={containerRef}
      className="bg-white/90 backdrop-blur-sm p-2 rounded-lg shadow-lg border border-gray-200 text-xs min-w-[220px] max-w-[280px] z-[1000]"
    >
      <div className="flex justify-between items-center mb-1">
...
function PointInfoPanel({ pointInfo, setPointInfo }) {
  ...
  return (
    <div 
      ref={containerRef}
      className="absolute bottom-10 left-3 z-[1000] bg-card/95 text-card-foreground p-3 rounded-lg shadow-xl border border-border w-72 text-sm"
    >
