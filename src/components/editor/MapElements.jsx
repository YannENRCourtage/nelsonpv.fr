...
function PrixCarburantLayerManager({ layersRef, activeLayers }) {
  const map = useMap();
  const active = activeLayers?.has('prixCarburant');
  const loadedIds = useRef(new Set());
  const layerGroupRef = useRef(L.featureGroup());

  useEffect(() => {
    if (!layersRef.current) return;
    layersRef.current['prixCarburant'] = layerGroupRef.current;
  }, [layersRef]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        data.results.forEach(item => {
          if (!item.geom || item.geom.lat === undefined) return;
          const latlng = [item.geom.lat, item.geom.lon];
          ...
          let pricesHtml = '';
          const carburants = [
            { id: 'gazole', label: 'Gazole', prix: item.gazole_prix, maj: item.gazole_maj },
            { id: 'sp95', label: 'SP95', prix: item.sp95_prix, maj: item.sp95_maj },
            { id: 'sp98', label: 'SP98', prix: item.sp98_prix, maj: item.sp98_maj },
            { id: 'e10', label: 'SP95-E10', prix: item.e10_prix, maj: item.e10_maj },
            { id: 'e85', label: 'E85', prix: item.e85_prix, maj: item.e85_maj },
            { id: 'gplc', label: 'GPLc', prix: item.gplc_prix, maj: item.gplc_maj }
          ];
      ...
      console.error("Error fetching Prix Carburant data", err);
    }
  };
