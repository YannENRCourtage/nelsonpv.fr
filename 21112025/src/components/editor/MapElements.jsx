...
// --- Clé API IGN ---
// CORRECTION : L'e-mail de l'IGN confirme que "essentiels" est la clé publique.
const VOTRE_CLE_IGN = "essentiels";
// 👆 Plus besoin de clé privée ! 👆
  ...
  return (
    <LayerGroup>
      {features.map((f) => {
        ...
        // ====================================================================
        // CORRECTION DE LA FAUTE DE FRAPPE (</Tooltip> en trop supprimé)
        // ====================================================================
        ...
        const canvas = await html2canvas(map.getContainer(), {
          ...
          onclone: (doc) => {
            ...
            if (leafletPane) {
              ...
              // Cible plus précisément le SVG dans le panneau overlay
              const svgPane = doc.querySelector('.leaflet-overlay-pane svg');
              if(svgPane) {
...
function PointInfoPanel({ pointInfo, setPointInfo }) {
  ...
  }, [pointInfo, setPointInfo, VOTRE_CLE_IGN]); // 'targetPos' retiré, n'était pas défini ici
