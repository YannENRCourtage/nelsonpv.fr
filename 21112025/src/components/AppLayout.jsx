import React from 'react';
    ...
    export const generatePdfForProject = async (projectData) => {
        ...
        if (validCaptures.length > 0) {
            ...
            legendContainer.style.width = "1123px"; // Largeur A4 paysage en pixels (297mm * ~3.78)
            ...
            const legendPdfWidth = doc.internal.pageSize.getWidth() - 30; // Marges de 15mm
            ...
            for (let i = 0; i < validCaptures.length; i++) {
                ...
                // Hauteur max de l'image = hauteur page - marge haut (20) - marge bas (10) - hauteur légende
                const capturePdfHeight = doc.internal.pageSize.getHeight() - 30 - legendPdfHeight;
                ...
                const y = 20; // 20mm du haut
    
                doc.addImage(captureDataUrl, 'PNG', x, y, imgWidth, imgHeight);

                // --- AJOUT DE LA LÉGENDE SOUS L'IMAGE ---
                doc.addImage(legendImgData, 'PNG', 15, legendY, legendPdfWidth, legendPdfHeight);
            }
        }
        ...
        const formatProjectName = () => {
            ...
            if (!p.name) {
                return client.lastName ? client.lastName.toUpperCase() : 'Projet';
            }
    ...
    function Header() {
      ...
      const getProjectTitle = () => {
        ...
        const title = `${p.name || ''} ${p.zip || ''} ${p.city || ''}`.trim();
        if (!p.name) {
             // Fallback si p.name est vide
            return (client.lastName ? client.lastName.toUpperCase() : 'PROJET') + (p.zip ? ` ${p.zip}` : '') + (p.city ? ` ${p.city}` : '');
        }
        return title.toUpperCase() || 'PROJET';
      }
